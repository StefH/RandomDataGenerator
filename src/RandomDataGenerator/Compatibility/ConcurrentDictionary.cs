...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary //, IReadOnlyDictionary<TKey, TValue>
    {
                ...
                lock (tables.m_locks[lockNo])
                {
                    ...
                    for (Node curr = tables.m_buckets[bucketNo]; curr != null; curr = curr.m_next)
                    {
                        ...
                        if (comparer.Equals(curr.m_key, key))
                        {
                            ...
                            if (prev == null)
                            {
                                RandomDataGenerator.Compatibility.Volatile.Write<Node>(ref tables.m_buckets[bucketNo], curr.m_next);
                            }
                            else
                            {
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
            ...
            Node n = RandomDataGenerator.Compatibility.Volatile.Read<Node>(ref tables.m_buckets[bucketNo]);
        ...
        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int index)
        {
            ...
            if (index < 0) throw new ArgumentOutOfRangeException("index", string.Format("The index argument is less than zero."));
        ...
        public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
        {
            ...
            for (int i = 0; i < buckets.Length; i++)
            {
                ...
                Node current = RandomDataGenerator.Compatibility.Volatile.Read<Node>(ref buckets[i]);
        ...
        private bool TryAddInternal(TKey key, TValue value, bool updateIfExists, bool acquireLock, out TValue resultingValue)
        {
            while (true)
            {
                ...
                {
                    if (acquireLock)
                        RandomDataGenerator.Compatibility.Monitor.Enter(tables.m_locks[lockNo], ref lockTaken);
                    ...
                    RandomDataGenerator.Compatibility.Volatile.Write<Node>(ref tables.m_buckets[bucketNo], new Node(key, value, hashcode, tables.m_buckets[bucketNo]));
                    checked
                    {
                        tables.m_countPerLock[lockNo]++;
                    }
        ...
        */
        ...
        {
            if (keyValuePair.Key == null) throw new ArgumentNullException(string.Format("TKey is a reference type and item.Key is null."));
        ...
        {
            if (key == null) throw new ArgumentNullException("key");
            if (!(key is TKey)) throw new ArgumentException(string.Format("The key was of an incorrect type for this dictionary."));
        ...
        object IDictionary.this[object key]
        {
            ...
            set
            {
                ...
                if (!(key is TKey)) throw new ArgumentException(string.Format("The key was of an incorrect type for this dictionary."));
                if (!(value is TValue)) throw new ArgumentException(string.Format("The value was of an incorrect type for this dictionary."));
        ...
        {
            get { return DEFAULT_CONCURRENCY_MULTIPLIER * RandomDataGenerator.Compatibility.PlatformHelper.ProcessorCount; }
        }
        ...
        private void AcquireLocks(int fromInclusive, int toExclusive, ref int locksAcquired)
        {
            ...
            for (int i = fromInclusive; i < toExclusive; i++)
            {
                ...
                try
                {
                    RandomDataGenerator.Compatibility.Monitor.Enter(locks[i], ref lockTaken);
                }
                finally
                {
                    if (lockTaken)
